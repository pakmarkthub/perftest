...
static CUresult gpu_mem_alloc(gpu_mem_handle_t *handle, const size_t size, bool aligned_mapping, bool set_sync_memops)
{
...
static CUresult gpu_mem_free(gpu_mem_handle_t *handle)
{
    CUresult ret = CUDA_SUCCESS;
...
int main(int argc, char *argv[])
{
    ...
    user_param.verb    = GVERBS;
    user_param.tst     = SANITY;
    ...
    if (cuda_init(&user_param))
        return FAILURE;

    control_in_vidmem_result = control_in_vidmem_test(context);
    gpu_uar_mapping_result = gpu_uar_mapping_test(context);
